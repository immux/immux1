...
use crate::cortices::mongo::ops::op_reply::{
    deserialize_op_reply_response_flags, serialize_op_reply, OpReply, OpReplyResponseFlags,
};
            ...
            if cstr_eq_str(&op_query.full_collection_name, ADMIN_QUERY) {
                if let Ok(config) = load_config(core) {
                    ...
                    let mut op_reply = OpReply {
                        ...
                        response_flags: deserialize_op_reply_response_flags(
                            &OpReplyResponseFlags {
                    ...
                    match serialize_op_reply(&op_reply) {
                        ...
                        Ok(vec) => {
                            ...
                            match serialize_op_reply(&op_reply) {
                                ...
                                Ok(actual_data) => {
                                    return ExceptionQueryHandlerResult::Exceptional(
                                        (Ok(Some(actual_data))),
                                    );
                                }
                            }
